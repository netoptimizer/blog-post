...
index_key. As we will explore in the next section, this flexibility is used
for CPU steering. The map-redirect is also responsible for creating a bulk
effect (as drivers are required to call a xdp_flush operation when NAPI-poll
budget ends).
